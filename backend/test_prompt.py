import sys
import os
import json

backend_dir = os.path.dirname(os.path.abspath(__file__))
sys.path.append(backend_dir)

from graph_engine import route_engine

def generate_prompt(start_node_id, end_node_id):
    route_result = route_engine.get_safest_route(start_node_id, end_node_id, user_type="blind")
    route_info_text = ""
    if route_result.get("success") and route_result.get("routes"):
        route_info_text = f"【系统已为您算出多条安全的路线，请你严格按照以下步骤播报，绝对不要使用东南西北，请用左右转】：\n"
        
        for r_idx, route in enumerate(route_result['routes']):
            if r_idx == 0:
                prefix = "推荐路线："
            elif r_idx == 1:
                prefix = "备选路线一："
            elif r_idx == 2:
                prefix = "备选路线二："
            else:
                prefix = f"路线{r_idx+1}："
                
            merged_steps = []
            for i, step in enumerate(route['steps']):
                dist = step['distance']
                turn = step.get('turn_instruction', '直行')
                to_name = step['to_name']
                
                if "路口" in to_name:
                    display_name = "一个路口"
                elif "转角" in to_name:
                    display_name = "一个转角"
                else:
                    display_name = to_name

                if i > 0 and turn == "直行" and to_name != route_result['end_name']:
                    merged_steps[-1]['distance'] += dist
                    merged_steps[-1]['to_name'] = display_name
                else:
                    merged_steps.append({
                        "turn": turn,
                        "distance": dist,
                        "to_name": display_name,
                        "is_endpoint": to_name == route_result['end_name']
                    })
            
            step_descriptions = []
            for i, m in enumerate(merged_steps):
                if i == 0:
                    if m['turn'] == "出发" or m['turn'] == "直行":
                        action_text = f"从起点出发，朝着【{m['to_name']}】的方向走"
                    else:
                        action_text = f"从起点出发，先{m['turn']}，朝着【{m['to_name']}】的方向走"
                        
                    if m['is_endpoint']:
                        desc = f"{action_text}大约{m['distance']:.0f}米，就直接到达{m['to_name']}了。"
                    else:
                        desc = f"{action_text}大约{m['distance']:.0f}米，到达{m['to_name']}。"
                else:
                    if m['is_endpoint']:
                        desc = f"然后{m['turn']}，走大约{m['distance']:.0f}米，就到达{m['to_name']}了。"
                    else:
                        desc = f"然后{m['turn']}，走大约{m['distance']:.0f}米，到达{m['to_name']}。"
                step_descriptions.append(desc)
            
            route_info_text += prefix + " ".join(step_descriptions) + f" (预计耗时{route['estimated_minutes']}分钟, 难度得分{route['difficulty_score']}分)\n"
    
    print(route_info_text)

generate_prompt('node_xm_glangyu_21', 'node_xm_glangyu_01')